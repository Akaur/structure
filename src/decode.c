...
{
  seed = 0;
  unsigned int sum1 = seed & 0xffff;
  unsigned int sum2 = seed >> 0x10;

  while (n!=0)
    {
      unsigned int chunksize = min (0x15b0, n);
      n -= chunksize;
...
// CRC
 ...
 unsigned int ckr, ckr2;
       ...
       ckr2 = bit_ckr8(0xc0c1, hdl_dat.chain + duabyte, section_size);
 
       if (ckr != ckr2)
         {
           printf("sectionobj %d crc todo ckr:%x ckr2:%x\n",
                   dwg->header.section[2].number, ckr, ckr2);
           return -1;
         }
...
decode_R2004(Bit_Chain* dat, Dwg_Data * dwg)
{
  ...
  if (loglevel)
    {
      ...
      LOG_TRACE("Second header data address: %X\n",
          (unsigned long int) _2004_header_data.fields.second_header_data_address)
  ...
  checksum (0, &ss.fields.comp_data_size, ss.fields.comp_data_size);
